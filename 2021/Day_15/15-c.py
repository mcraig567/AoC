...
class PQueue:
	...
	def isEmpty(self):
		return len(self.queue) == 1  # We keep a never-ending start entry, so length of 1 = empty
	...
	def bubbleDown(self, i):
		...
		while i != 1 and self.queue[self.parent(i)]["shortest_path"] > movingVal:
	...
	def getIndex(self, loc):
		if loc == (0,0): # Don't want to catch the (0,0) at index 0
			return 1
...
def get_neighbours(location, risk_dict):
	...
	# Using this gives correct answer
	...
	# Using this gives incorrect answer (1 high)
	left = risk_dict.get((row, col - 1), None)
	if left:
		neighbour_list.append(left)
	...
	for neighbour in neighbours:
		if not neighbour["visited"]:
			...
			if distance < neighbour["shortest_path"]:
				...
				# We only visit a node once, so if a node isn't in the queue,
				# that's because it hasn't been visited yet

				if queue.isIn(neighbour["id"]):
					index = queue.getIndex(neighbour["id"])
				...
				#print("Adding to queue: ", neighbour["id"], " - ", neighbour['shortest_path'])
...
shortest_queue.insert(large_map[beginning]) # Add twice so that queue index starts at 1
